...
-(id)initWithViewController:(UIViewController*)viewController contentSize:(CGSize)contentSize {
	return [self initWithViewController:viewController contentSize:contentSize delegate:nil];
}

-(id)initWithViewController:(UIViewController*)viewController contentSize:(CGSize)contentSize delegate:(id<FPPopoverControllerDelegate>)delegate
{
    self = [super init];
    if(self)
    {
...
-(void)presentPopoverFromPoint:(CGPoint)fromPoint inView:(UIView *)view
{
    ...
    _contentView.relativeOrigin = [_fromView convertPoint:fromPoint toView:_contentView];
...
- (void)presentPopoverFromView:(UIView *)fromView inView:(UIView *)inView
{
    SAFE_ARC_RELEASE(_fromView);
    _fromView = SAFE_ARC_RETAIN(fromView);
    _parentView = inView;
    [self setupView];
    [self presentPopoverFromPoint:[self originFromView:_fromView] inView:inView];
}
...
-(void)setFromPoint:(CGPoint)origin
{
    _fromPoint = origin;
}
...
-(CGRect)bestArrowDirectionAndFrameFromView:(UIView*)v
{
    ...
    float width = 2.0f;
    float height = 2.0f;
    CGPoint p = CGPointMake(self.fromPoint.x, self.fromPoint.y);
...
//    }
    ...
    _contentView.frame = [_fromView.superview convertRect:r toView:_parentView];

//    self.origin = CGPointMake(p.x + v.frame.size.width/2.0, p.y + v.frame.size.height/2.0);
    _contentView.relativeOrigin = [_fromView.superview convertPoint:self.fromPoint toView:_contentView];
